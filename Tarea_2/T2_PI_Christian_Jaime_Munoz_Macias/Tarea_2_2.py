...
def metodo_Kaczmarz(K,b,x0,tau,tol=1e-6,max_iter=1000):
    x=np.copy(x0)
    for i in range(max_iter):
        pos=np.random.randint(0,K.shape[0])
        a=K[pos]
        oper=b[pos]-np.dot(a,x)
        norm_a=np.linalg.norm(a)**2

        x+=oper*a/norm_a
        if np.linalg.norm(K@x-b)<tau*tol:
            print(f"iteracion de Kaczmarz convergió en {i} iteraciones.")
            break
...
if __name__=="__main__":
    ...
    for n_i in n:
        for SNR_i in SNR:
            ...
            #x_ini=np.random.uniform(0,1,size=len(b_rui))
            x_ini=np.zeros_like(b_rui)
            f_ob_kacz=metodo_Kaczmarz(K,b_rui,x_ini,tau=1.01,tol=noise_level,max_iter=10000)
       
            plt.figure()

            plt.title(rf"$n={n_i},\,SNR={SNR_i}$.")
            plt.plot(x,f_ev,label=r"$f_{true}$")
            plt.plot(x,f_ob_kacz,label=r"$f_{Kaczmarz}$")
            plt.grid(True,alpha=0.3)
            plt.legend()

            plt.tight_layout()
            plt.savefig(f"Tarea_2_2_n{n_i}_SNR{SNR_i}.pdf")
            plt.show()
